...
unsigned char GET[58]="GET /firesafty/requestData/request.php?field=M1&field1=",f2[9]="&field2=",f3[9]="&field3=";
...
int main(void)
{
  ...
  _delay_us(1000);
   UART_Tstring(x);
  uint16_t a = readAnalag(5);
  uint64_t k = ( a/1024.0)*5000; 
  uint16_t cel_i = k/10;
  itoa(cel_i,x,10);
  _delay_ms(200);
  UART_Tstring(f2);
  _delay_us(2000);
  uint16_t b = readAnalag(1);
  itoa(b,y,10);
  _delay_us(200);
  UART_Tstring(y);
  _delay_us(100);
